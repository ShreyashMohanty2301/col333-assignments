...
// }
// pair<Solution,ProblemData> generate_neighbours_swap(Solution &sol,const ProblemData &prob , int i){
//     int i2 = -1;
...
//     return dist;
// } 
// pair<Solution,ProblemData> generate_neighbours_add(Solution &sol, const ProblemData & prob,int i){
//     Solution neigh = sol;
//     int i1 = -1;
//     for(int j = 0;j<(int)sol.size();j++){
//         if(sol[j].helicopter_id-1 == i){
...
//                 // Solution neigh;
//                 // ProblemData neprob;
//                 // if(move_dist(gen10)<0.7){
//                     pair<Solution,ProblemData>neigh1 = generate_neighbours_add(finsol,prob1,i);
...
//                     // }
//                     // pair<Solution,ProblemData>ni = generate_neighbours_add(finsol, prob, i);
//                     // neigh = ni.first;
//                     // neprob = ni.second;
//                 // }
//                 // else{
//                     pair<Solution,ProblemData>neigh = generate_neighbours_swap(finsol,prob1,i);
//                     double h = h_val(neigh.first,neigh.second);
//                     if(h>currhval){
//                         finsol = neigh.first;
//                         prob = neigh.second;
//                         ok = true;
...
#include <cmath>     // For std::min, std::exp
...
/**
 * @brief Calculates the total weight of all packages picked up for a single trip.
 */
double calculate_trip_weight(const Trip& trip, const ProblemData& problem) {
...
/**
 * @brief Calculates the total distance of a single trip.
 */
double calculate_trip_distance(const Trip& trip, const Helicopter& helicopter, const ProblemData& problem) {
    if (trip.drops.empty()) {
        return 0.0;
    }
    double total_dist = 0.0;
    const Point& home_city_coords = problem.cities[helicopter.home_city_id];
    total_dist += distance(home_city_coords, problem.villages[trip.drops.front().village_id].coords);
    for (size_t i = 0; i < trip.drops.size() - 1; ++i) {
        total_dist += distance(problem.villages[trip.drops[i].village_id].coords, problem.villages[trip.drops[i + 1].village_id].coords);
    }
    total_dist += distance(problem.villages[trip.drops.back().village_id].coords, home_city_coords);
    return total_dist;
}


/**
 * @brief Calculates the heuristic value (score) of a given solution.
 */
double h_val(Solution &sol, const ProblemData &problem) {
    double total_value = 0.0;
    double total_cost = 0.0;
    vector<vector<double>> village_drops(problem.villages.size(), vector<double>(3, 0.0));
    ...
    for (const auto &plan : sol) {
        for (const auto &trip : plan.trips) {
            ...
            const auto& helicopter = problem.helicopters[plan.helicopter_id];
            double trip_dist = calculate_trip_distance(trip, helicopter, problem);
            total_cost += helicopter.fixed_cost + helicopter.alpha * trip_dist;

            for (const auto &drop : trip.drops) {
                village_drops[drop.village_id][0] += drop.dry_food;
                village_drops[drop.village_id][1] += drop.perishable_food;
                village_drops[drop.village_id][2] += drop.other_supplies;
            }
        }
    }

    for (size_t i = 0; i < village_drops.size(); ++i) {
        const auto& village = problem.villages[i];
        double perishable_delivered = village_drops[i][1];
        double dry_food_delivered = village_drops[i][0];
        double other_supplies_delivered = village_drops[i][2];

        double max_food_need = 9.0 * village.population;

        double effective_perishable = min(max_food_need, perishable_delivered);
        double remaining_food_need = max_food_need - effective_perishable;
        double effective_dry_food = min(remaining_food_need, dry_food_delivered);
        double effective_other = min((double)village.population, other_supplies_delivered);

        total_value += effective_dry_food * problem.packages[0].value;
...
/**
 * @brief Checks if a specific trip is valid regarding per-trip distance and weight constraints.
 */
bool is_trip_valid(const Trip& trip, const Helicopter& helicopter, const ProblemData& prob) {
    if (calculate_trip_weight(trip, prob) > helicopter.weight_capacity + 1e-9) { // Add epsilon for float comparison
        return false;
    }
    return calculate_trip_distance(trip, helicopter, prob) <= helicopter.distance_capacity;
}


/**
 * @brief Generates a new solution state by attempting to add a drop to a trip, with incremental distance checking.
 */
void generate_neighbours_add(Solution& sol, vector<double>& total_distances, const ProblemData& prob, int helicopter_id, mt19937& gen) {
    int plan_idx = -1;
    for (size_t i = 0; i < sol.size(); ++i) {
        if (sol[i].helicopter_id == helicopter_id) {
            plan_idx = i;
            break;
        }
    }
    if (plan_idx == -1) {
        sol.push_back({helicopter_id, {}});
        plan_idx = sol.size() - 1;
    }

    uniform_int_distribution<int> trip_dist(0, sol[plan_idx].trips.size());
    int trip_idx = trip_dist(gen);

    if (trip_idx == (int)sol[plan_idx].trips.size()) {
        sol[plan_idx].trips.push_back({});
    }

    const auto& helicopter = prob.helicopters[helicopter_id];
    const Trip original_trip = sol[plan_idx].trips[trip_idx]; 
    Trip temp_trip = original_trip;

    set<int> available_villages;
    for(size_t i = 0; i < prob.villages.size(); ++i) available_villages.insert(i);
    for(const auto& drop : temp_trip.drops) available_villages.erase(drop.village_id);

    if (available_villages.empty()) return;
    
    uniform_int_distribution<int> village_dist(0, available_villages.size() - 1);
    auto it = available_villages.begin();
    std::advance(it, village_dist(gen));
    int new_village_id = *it;

    double current_weight = calculate_trip_weight(temp_trip, prob);
    double remaining_weight = helicopter.weight_capacity - current_weight;
    ...
    uniform_real_distribution<double> food_dist(0.0, remaining_weight);
    double more_food_weight = food_dist(gen);

    uniform_real_distribution<double> ratio_dist(0.0, 1.0);
    double r1 = ratio_dist(gen), r2 = ratio_dist(gen), r3 = ratio_dist(gen);
    double total_ratio = r1 + r2 + r3;
    if (total_ratio == 0) return;
    r1 /= total_ratio; r2 /= total_ratio; r3 /= total_ratio;
    
    int d = (r1 * more_food_weight) / prob.packages[0].weight;
    int p = (r2 * more_food_weight) / prob.packages[1].weight;
    int o = (r3 * more_food_weight) / prob.packages[2].weight;
    
    new_drop.dry_food = d;
    new_drop.perishable_food = p;
    new_drop.other_supplies = o;
    
    uniform_int_distribution<int> insert_pos_dist(0, temp_trip.drops.size());
    int pos = insert_pos_dist(gen);
    temp_trip.drops.insert(temp_trip.drops.begin() + pos, new_drop);

    temp_trip.dry_food_pickup += d;
    temp_trip.perishable_food_pickup += p;
    temp_trip.other_supplies_pickup += o;

    if (is_trip_valid(temp_trip, helicopter, prob)) {
        double old_dist = calculate_trip_distance(original_trip, helicopter, prob);
        double new_dist = calculate_trip_distance(temp_trip, helicopter, prob);
        double delta_dist = new_dist - old_dist;
        
        if (total_distances[helicopter_id] + delta_dist <= prob.d_max) {
            sol[plan_idx].trips[trip_idx] = temp_trip;
            total_distances[helicopter_id] += delta_dist;
        }
    }
}

/**
 * @brief Generates a new solution by swapping two drops, with incremental distance checking.
 */
void generate_neighbours_swap(Solution& sol, vector<double>& total_distances, const ProblemData& prob, int helicopter_id, mt19937& gen) {
    int plan_idx = -1;
    for (size_t i = 0; i < sol.size(); ++i) {
        if (sol[i].helicopter_id == helicopter_id) {
            plan_idx = i;
    ...
    if (plan_idx == -1 || sol[plan_idx].trips.empty()) return;

    uniform_int_distribution<int> trip_dist(0, sol[plan_idx].trips.size() - 1);
    int trip_idx = trip_dist(gen);
    
    if (sol[plan_idx].trips[trip_idx].drops.size() < 2) return;

    uniform_int_distribution<int> drop_dist(0, sol[plan_idx].trips[trip_idx].drops.size() - 1);
    int idx1 = drop_dist(gen);
    int idx2 = drop_dist(gen);
    while (idx1 == idx2) {
        idx2 = drop_dist(gen);
    }
    ...
    const Trip original_trip = sol[plan_idx].trips[trip_idx];
    Trip temp_trip = original_trip;
    swap(temp_trip.drops[idx1], temp_trip.drops[idx2]);
...
/**
 * @brief Generates a random valid solution and its initial distances.
 */
pair<Solution, vector<double>> Random_state_generator(const ProblemData& problem, const vector<int>& close, mt19937& gen) {
    Solution state;
    vector<double> total_distances(problem.helicopters.size(), 0.0);
    
    for (int i = 0; i < 200; ++i) {
        uniform_int_distribution<int> helicopter_dist(0, problem.helicopters.size() - 1);
        int random_helicopter_id = helicopter_dist(gen);
        generate_neighbours_add(state, total_distances, problem, random_helicopter_id, gen);
    }
    return {state, total_distances};
}


/**
 * @brief Implements a simulated annealing algorithm with efficient distance checks.
 */
Solution solve(const ProblemData& problem) {
    cout << "Starting solver..." << endl;
    ...
    auto buffer = milliseconds(100); 
    auto deadline = start_time + allowed_duration - buffer;

    mt19937 gen(chrono::steady_clock::now().time_since_epoch().count());
    
    vector<int> close(problem.helicopters.size());
    for(size_t i = 0; i < problem.helicopters.size(); i++){
        int curr = -1;
        double dist = std::numeric_limits<double>::max();
        for(size_t j = 0; j < problem.villages.size(); j++){
            if(distance(problem.villages[j].coords, problem.cities[problem.helicopters[i].home_city_id]) < dist){
                dist = distance(problem.villages[j].coords, problem.cities[problem.helicopters[i].home_city_id]);
                curr = j;
            }
        }
        close[i] = curr;
    ...
    auto initial_state = Random_state_generator(problem, close, gen);
    Solution finsol = initial_state.first;
    vector<double> helicopter_total_distances = initial_state.second;

    Solution bestsol = finsol;
    double currhval = h_val(finsol, problem);
    double best = currhval;
    ...
    while (steady_clock::now() < deadline) {
        ...
        Solution neigh = finsol; // Start with a copy of the current solution
        vector<double> neigh_distances = helicopter_total_distances; // and its distances

        if(move_dist(gen) < 0.7){
             generate_neighbours_add(neigh, neigh_distances, problem, i, gen);
        } else {
             generate_neighbours_swap(neigh, neigh_distances, problem, i, gen);
        }

        double h = h_val(neigh, problem);
        double score_diff = h - currhval;

        if (score_diff > 0 || acceptance_dist(gen) < exp(score_diff / temperature)) {
            finsol = neigh;
            currhval = h;
            helicopter_total_distances = neigh_distances; // Commit the new distances
        }
